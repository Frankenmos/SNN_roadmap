...
def test_training_analyzer_uses_smart_action_semantics_from_effective_config(tmp_path):
    run_dir = tmp_path / "SMART-1"
    run_dir.mkdir()
    db_path = run_dir / "training_logs.db"
    conn = initialize_db(str(db_path))
    with conn:
        ...
        step_rows = [
            (1, idx, 1 if idx < 7 else 0, 10, 11, 1.0, float(idx + 1))
            for idx in range(10)
        ]
        conn.executemany(
            "INSERT INTO steps "
    ...
    conn.close()
    (run_dir / "effective_config.json").write_text(
        json.dumps({"model": {"action_dim": 2, "vector_input_dim": 32}}),
        encoding="utf-8",
    )
