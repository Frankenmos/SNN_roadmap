...
def _two_attacks_then_idle(ppo, net):
    ...
    _store_step(ppo, net, action=POLICY_ACTION_RIGHT_CLICK, engagement_feedback=False, reward=50.0, done=False)
    # step1: RIGHT_CLICK (its feedback confirms step0 engaged; admitted via step2)
    _store_step(ppo, net, action=POLICY_ACTION_RIGHT_CLICK, engagement_feedback=True, reward=50.0, done=False)
    # step2: NO_OP (its feedback confirms step1 engaged); never admitted itself
    _store_step(ppo, net, action=POLICY_ACTION_NO_OP, engagement_feedback=True, reward=0.0, done=True)
...
def _log_prob_of_stored_action(ppo, net):
    ...
    with torch.no_grad():
        ...
        logits = ppo._mask_action_logits(
            ppo.policy_net.action_head(latent), obs.meta_vec.to(net.device),
        )
...
def test_sil_admits_committed_attacks_and_runs_pass():
    ...
    ppo = PPO(
        net, lr=1e-4, total_updates=0, lr_min=0.0, tbptt_window=4,
        sil_enabled=True, sil_buffer_size=100, sil_batch_fraction=1.0,
        sil_coef=0.5,
    )
...
def test_sil_disabled_leaves_update_unchanged():
    ...
    ppo = PPO(
        net, lr=1e-4, total_updates=0, lr_min=0.0, tbptt_window=4,
        sil_enabled=False, sil_coef=0.5,
    )
    _two_attacks_then_idle(ppo, net)
...
def test_sil_pass_increases_log_prob_of_stored_good_action():
    ...
    net = _make_net(action_dim=3)
    ppo = PPO(
        net, lr=5e-3, total_updates=0, lr_min=0.0, tbptt_window=4,
        sil_enabled=True, sil_buffer_size=100, sil_batch_fraction=1.0,
        sil_coef=2.0,
    )
