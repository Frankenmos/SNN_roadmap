...
Status: historical review baseline used as a checklist of prior findings.
Current repo runtime has since fixed the "policy_mask" ambiguity, moved update-before-eval,
and introduced the positional/spatial-head repair; keep this doc as reference for older
recommendation history, not as an unresolved action list.

Problems found so far and recommended repairs
Scope: policy protocol, spiking policy, PPO trainer, agent wrapper, and training loop provided in chat.
...
 
Literature cross-check and compact patch appendix
Purpose. This appendix separates recommendations that are strongly supported by published work from recommendations that are primarily code-review and experiment-hygiene judgments. It then turns the highest-confidence items into a compact patch plan for the current branch.
A. Literature cross-check
A1. Strong support from prior work
•	Keep spatial structure alive for spatial actions. The strongest literature support is here. The SC2LE paper explicitly warns that discarding spatial structure before inferring actions can be detrimental for StarCraft because spatial actions operate in the same space as the input. Its FullyConv baseline preserves spatial resolution and predicts spatial actions directly from a resolution-preserving spatial state, rather than from a fully pooled latent.
•	Add explicit positional information to flattened spatial tokens. This is also well supported. Attention Is All You Need adds positional encodings because self-attention alone does not know token order or location. In vision, DETR gives a very close architectural precedent to your current design: it flattens CNN features into a sequence and adds 2D spatial positional encodings before transformer attention.
•	Treat temporal-core choice as an ablation, not an article of faith. The literature supports comparing memory mechanisms rather than assuming one will dominate. AlphaStar combined a transformer-style torso with a deep LSTM core and autoregressive/pointer-style action heads. GTrXL later showed that stabilized transformers can match or exceed strong LSTM baselines in partially observable RL. That means your plan to compare SNN vs GRU/LSTM vs transformer later is literature-aligned.
•	Handle time limits deliberately. Pardo et al. show that time-limit semantics matter. If the episode cap is a true task horizon, remaining time belongs in the state to preserve the Markov property. If the cap is only a training convenience, the timeout should not be treated as an environment terminal and value bootstrapping should continue across the truncation.
A2. Important nuance from the literature
•	A joint 2D heatmap is attractive, but it is not the only literature-consistent repair. SC2LE explicitly models discretized x and y coordinates independently in one baseline, so the factorized x/y choice is not automatically wrong. The literature-backed problem is not factorization by itself; it is predicting x and y from an over-pooled global latent instead of from a spatial representation. For this branch, the minimum literature-aligned fix is to preserve a spatial map for the click head. Moving from factorized x/y to a joint 2D heatmap can come later.
•	There is no strong literature pressure to replace the CNN stem right now. Both DETR and AlphaStar use hybrid perception stacks rather than "pure weirdness everywhere". DETR uses a CNN backbone before transformer processing; AlphaStar uses separate visual/unit/vector encoders before the temporal core and action heads. That supports keeping a simple CNN stem as the stable bottom of the stack while experimenting in the temporal core and spatial head.
A3. Mostly engineering judgments rather than literature claims
•	Whether policy_mask should be actor-only or should also gate the critic.
•	Moving PPO updates before deterministic evaluation and best-checkpoint selection.
•	Adding stricter protocol assertions for meta-vector width and action-availability slices.
•	Checkpointing in-flight rollout buffers and recurrent state for exact resume semantics.
Interpretation. The literature backs the spatial-head and positional-encoding repairs strongly; it supports doing later SNN-vs-GRU-vs-transformer comparisons; and it clarifies the time-limit issue. The mask semantics, update ordering, and resume behavior remain primarily engineering decisions that should be made explicit in the repo.
B. Compact patch plan (minimum useful repair)
Goal. Make the baseline literature-aligned and easier to interpret without turning the whole stack into a new research project.
agent_core/spiking_policy.py
•	Add learned or projected 2D positional embeddings for the 7×7 spatial tokens before attention. A simple version is a small Linear(2, D) projection applied to normalized (x, y) coordinates.
•	Keep a structured spatial branch alive for localization. After attention / temporal integration, keep the spatial token slice as a 7×7×D map (or D×7×7 tensor) instead of only feeding a group-mean pooled vector to the click head.
•	Leave the value head and non-spatial action head on the pooled/global latent if you want, but change conditioned_spatial_head() so it consumes the structured spatial context plus the chosen high-level action.
•	For the first patch, it is acceptable to keep factorized x/y logits, but they should be derived from the structured spatial map rather than the 64-d pooled latent. A stronger later patch is a joint 2D heatmap or token-pointer head.
•	Optional clarity patch: rename num_steps to snn_micro_steps so it is harder to confuse SNN micro-time with environment-time memory.
agent_core/ppo_trainer.py
•	Thread the new spatial_context output through select_action() and replay, so training and acting use the same spatial head contract.
•	Add fail-fast assertions in store_transition() and set_final_next() that the stored PolicyInputBatch carries the recurrent state expected by replay and bootstrap.
•	Decide the semantics of policy_mask. If it means actor-only masking, rename it to actor_mask. If it means “do not learn from this transition,” then apply the same mask to the critic loss and reconsider how returns / GAE should treat masked steps.
train.py
•	Move maybe_run_policy_update() ahead of deterministic evaluation and best-checkpoint selection so eval reflects the freshest trained parameters.
•	Make time-cap semantics explicit. If steps_per_episode is a true finite-horizon task boundary, include remaining time in the observation or recurrent state. If it is only a training truncation, store a separate timeout/truncated flag and bootstrap through it instead of folding it into done.
protocol/config hygiene
•	Either lock attention_pool_size to 7 in this branch or derive protocol token-count constants from the same source of truth so saved-state and protocol assumptions cannot drift apart.
•	Remove silent fallbacks in the experiment branch: exact meta-vector width and structured action-availability slices should fail fast when wrong.
C. Recommended sequencing
1.	First repair the spatial head while keeping the CNN stem. This is the highest-confidence architecture fix.
2.	At the same time, make mask semantics and time-limit semantics explicit so failed runs are interpretable.
3.	Only after that compare temporal cores under the same observation protocol and same action head: current SNN core, GRU/LSTM core, GTrXL-style transformer core, and a no-memory baseline.
4.	Do not swap the CNN stem at the same time. The literature supports hybrid stems, and keeping one “boring” component makes the temporal-core comparison much more informative.
D. References used for the cross-check
5.	Vaswani, A. et al. Attention Is All You Need. NeurIPS 2017.
6.	Carion, N. et al. End-to-End Object Detection with Transformers. ECCV 2020.
7.	Vinyals, O. et al. StarCraft II: A New Challenge for Reinforcement Learning. arXiv 2017.
8.	Vinyals, O. et al. Grandmaster level in StarCraft II using multi-agent reinforcement learning. Nature 2019.
9.	Pardo, F. et al. Time Limits in Reinforcement Learning. ICML 2018.
10.	Parisotto, E. et al. Stabilizing Transformers for Reinforcement Learning. ICML 2020.
