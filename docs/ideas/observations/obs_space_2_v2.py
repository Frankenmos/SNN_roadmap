...
# ---- PySC2 colors.py fix for Python 3.11+ ----
...
from pysc2.lib import features  # noqa: E402,F401
...
class ObservationExtractor:
    ...
    def extract_observation(self, obs, last_action_token=None, update_stats=True):
        ...
        meta_vec = self._extract_meta_vector(obs, last_action_token)
    ...
    def peek_observation(self, obs, last_action_token=None):
        return self.extract_observation(obs, last_action_token=last_action_token, update_stats=False)
    ...
    def _extract_meta_vector(self, obs, last_action_token=None):
        ...
        # --- NEW: agent's own last action token ---
        if last_action_token is None:
            agent_last = np.zeros(4, dtype=np.float32)
        else:
            token = np.asarray(last_action_token, dtype=np.float32).reshape(-1)
            # pad to 4
            if token.size < 4:
                token = np.pad(token, (0, 4 - token.size))
            else:
                token = token[:4]
            # normalize x,y if they look like screen coords
            if token[1] > 1.0:
                token[1] = token[1] / 83.0
            if token[2] > 1.0:
                token[2] = token[2] / 83.0
            agent_last = token
        ...
        # NOTE: this makes meta_vec 4 dims larger than META_VECTOR_DIM.
        # Update META_VECTOR_DIM in policy_input.py to old + 4, or trim elsewhere.
        full = np.concatenate((base, agent_last)).astype(np.float32)

        return torch.as_tensor(full, dtype=torch.float32, device=self.device)
    ...
    def get_observation_dimensions(self, obs):
        del obs
        return SPATIAL_OBS_SHAPE, META_VECTOR_DIM + 4  # +4 for agent token
